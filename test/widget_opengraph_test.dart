...
void main() {
  group('WidgetOpenGraph', () {
    testWidgets('renders correctly with all data in non-production mode',
      ...
      await tester.pumpWidget(MaterialApp(
        home: Scaffold(
          body: WidgetOpenGraph(
            ...
            height: 200,
            isProduction:
                false, // Usar modo no-producción para evitar cargar imágenes de red
            borderRadius: 10,
          ),
        ),
      ));
    ...
    testWidgets('handles empty image URL in non-production mode',
        (WidgetTester tester) async {
      final data = OpenGraphEntity(
        title: 'Test Title',
        description: 'Test Description',
    ...
    testWidgets('applies different border radius values',
        (WidgetTester tester) async {
      final data = OpenGraphEntity(
        title: 'Test Title',
        description: 'Test Description',
      ...
      expect(padding.padding, const EdgeInsets.all(customBorderRadius / 2));

      final clipRRect = tester.widget<ClipRRect>(find.byType(ClipRRect).first);
      expect(clipRRect.borderRadius,
          BorderRadius.circular(customBorderRadius / 2));
    });
  });
}
